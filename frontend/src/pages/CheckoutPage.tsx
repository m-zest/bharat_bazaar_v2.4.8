    ...
    <div className="p-6 lg:p-8 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-center gap-2 mb-8">
        {[
          ...
          <div key={s.num} className="flex items-center gap-2">
            ...
            <span className={`text-sm font-medium hidden sm:block ${step >= s.num ? 'text-gray-100' : 'text-gray-500'}`}>
              {s.label}
            </span>
            {i < 2 && <div className={`w-12 h-0.5 ${step > s.num ? 'bg-orange-500' : 'bg-[#2a2a2d]'}`} />}
          </div>
        ))}
      </div>
      ...
      {step === 1 && (
        ...
        >
                ...
                <div className="sm:col-span-2">
                  ...
                  <div className="relative">
                    ...
                    <textarea
                      ...
                      rows={2}
                      className="w-full pl-10 pr-4 py-2.5 bg-[#141416] border border-[#333] rounded-xl text-sm text-gray-100 placeholder:text-gray-500 focus:ring-2 focus:ring-orange-500 outline-none resize-none"
                    />
                  </div>
                </div>
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 bg-[#141416] border border-[#333] rounded-xl text-sm text-gray-100 placeholder:text-gray-500 focus:ring-2 focus:ring-orange-500 outline-none"
                  />
                </div>
                <div>
          ...
          {/* Mini Summary */}
          <div className="bg-[#1a1a1d] rounded-xl border border-[#2a2a2d] p-5 h-fit">
            <h3 className="font-bold text-gray-100 mb-3 text-sm">Order ({items.length} items)</h3>
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {items.map(item => (
                <div key={item.id} className="flex justify-between text-sm">
                  <span className="text-gray-400 truncate flex-1 mr-2">{item.name} x{item.quantity}</span>
                  <span className="text-gray-100 font-medium whitespace-nowrap">&#8377;{(item.wholesalePrice * item.quantity).toLocaleString('en-IN')}</span>
                </div>
              ))}
            </div>
            <div className="border-t border-[#2a2a2d] mt-3 pt-3 flex justify-between font-bold text-gray-100">
              <span>Total</span>
      ...
      {step === 2 && (
        ...
        >
          <div className="bg-[#1a1a1d] rounded-xl border border-[#2a2a2d] p-6">
                ...
                <label
                  ...
                  className={`flex items-center gap-4 p-4 rounded-xl border-2 cursor-pointer transition-all ${
                    form.paymentMethod === method.id
                      ...
                      : 'border-[#2a2a2d] hover:border-[#333]'
                  }`}
                >
                  <input
                    type="radio"
                  ...
                  />
                  <method.icon className={`w-5 h-5 ${form.paymentMethod === method.id ? 'text-orange-500' : 'text-gray-500'}`} />
                  <div>
            ...
            {/* Order Summary */}
            <div className="mt-6 bg-[#141416] rounded-xl p-4 space-y-2 text-sm">
            ...
            <div className="flex gap-3 mt-6">
              <button
                onClick={() => setStep(1)}
                className="px-6 py-3 border border-[#2a2a2d] text-gray-300 rounded-xl hover:bg-white/[0.06] transition-colors font-medium"
              >
                Back
              </button>
              <button
                ...
                disabled={placing}
                className="flex-1 py-3 bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white font-semibold rounded-xl shadow-lg shadow-green-500/20 transition-all flex items-center justify-center gap-2 disabled:opacity-60"
              >
                {placing ? (
      ...
      {step === 3 && (
        ...
        >
          <div className="bg-[#1a1a1d] rounded-2xl border border-[#2a2a2d] p-8">
            <motion.div
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
            ...
            <p className="text-gray-400 mb-1">Your wholesale order has been confirmed</p>
            <div className="inline-flex items-center gap-2 bg-white/[0.03] rounded-lg px-4 py-2 mt-2 mb-6">
              <span className="text-sm text-gray-400">Order ID:</span>
              <span className="text-sm font-bold text-gray-100">{orderId}</span>
            </div>
            ...
            <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-4 text-left mb-6">
              ...
              <p className="text-green-400 font-semibold">2-3 business days</p>
              <p className="text-green-400 text-xs mt-1">Delivery to: {form.address}, {form.city}</p>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-3 border border-[#2a2a2d] text-gray-300 hover:bg-white/[0.06] rounded-xl transition-colors font-medium flex items-center justify-center gap-2"
              >
                <Truck className="w-4 h-4" />
                Track Order
              </button>
